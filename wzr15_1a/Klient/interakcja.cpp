...
#define MY_PORT_REC 1001
#define MY_PORT_SEND 1002
...
unicast_net *netSend;         // wsk do obiektu zajmujacego sie odbiorem komunikatow
unicast_net *netRec;          //   -||-  wysylaniem komunikatow
...
{
  unicast_net *netRecive=(unicast_net*)ptr;  // wskaŸnik do obiektu klasy multicast_net
  ...
  unsigned long t;
	while(1)
	{
		...
		rozmiar = netRecive->reciv((char*)&ramka,&t,sizeof(Ramka));   // oczekiwanie na nadejœcie ramki 
		stan = ramka.stan;
		...
		if (ramka.typ == 1){
			...
			if (stan.iID == pMojObiekt->iID){
				int msgboxID = MessageBox(
					NULL,
					"Kolejny gracz chce do³¹czyæ. Zgadzasz sie?",
					"Nowy gracz",
				...
				Ramka ramkaGlos;
				ramkaGlos.typ = 2;
				ramkaGlos.new_iID = ramka.new_iID;
				switch (msgboxID)
				{
				case IDYES:
					ramkaGlos.glos = 1;
				...
				int iRozmiar = netSend->send((char*)&ramkaGlos, SERVER_IP, sizeof(Ramka));
			}
...
void PoczatekInterakcji()
{
  ...
  // multi_send = new multicast_net("224.12.12.20",10001);       // obiekt do wysy³ania ramek
   netSend = new unicast_net(MY_PORT_SEND);
   netRec = new unicast_net(MY_PORT_REC);
   ...
   threadReciv = CreateThread(
          ...
          (void *)netRec,               // argument to thread function
...
void Cykl_WS()
{
   ...
   int iRozmiar = netSend->send((char*)&ramka,SERVER_IP,sizeof(Ramka));          
}
